...
/*GAMES*/
DrkBox.addCommand({pattern: "game ?(.*)", fromMe: true}, (async (message, match) => {
		if (match[1] === "1") {
...
/*TOOLS*/
DrkBox.addCommand({pattern: "tool ?(.*)", fromMe: true}, (async (message, match) => {
	// Launcher
		if (match[1] === "1") {
...
/*GAMES*/
DrkBox.addCommand({pattern: "game ?(.*)", fromMe: false}, (async (message, match) => {
		if (match[1] === "1") {
			await message.sendMessage('💎 *1v1 Lol Mod* 💎\n##desc\n⚠️ ##aviso\n📌 https://ouo.io/7rUiaB', MessageType.text, {quoted: message.data});
		}
		else if (match[1] === "2") {
