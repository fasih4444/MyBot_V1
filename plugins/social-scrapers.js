...
DrkBox.addCommand({ pattern: 'twt ?(.*)', fromMe: wk,  dontAddCommandList: true, desc: "Descargas de twitter" }, async (message, match) => {
    ...
    }).catch(async (err) => { await message.sendMessage(errorMessage("🤖 Parece que hay un error!0" ))
}));
...
DrkBox.addCommand({pattern: 'igdown ?(.*)', fromMe: wk}, (async (message, match) => {
    ...
    /*
    DrkBox.addCommand({ pattern: 'igdown ?(.*)', fromMe: wk, desc: Lang.DESC}, async (message, match) => {
	 const link = match[1]
